...
def _get_profile(service, region, key, keyid, profile):
    if profile:
        if isinstance(profile, six.string_types):
            _profile = __salt__['config.get'](profile)
        elif isinstance(profile, dict):
            _profile = profile
        key = _profile.get('key', None)
        keyid = _profile.get('keyid', None)
        region = _profile.get('region', region or None)
    if not region and __salt__['config.get'](service + '.region'):
        region = __salt__['config.get'](service + '.region')

    if not region:
        region = 'us-east-1'
    if not key and __salt__['config.get'](service + '.key'):
        key = __salt__['config.get'](service + '.key')
    if not keyid and __salt__['config.get'](service + '.keyid'):
        keyid = __salt__['config.get'](service + '.keyid')
